...
export const createMonth = createServerFn({ method: 'POST' as const })
  .validator((data: { year: number; monthNo: number; userId?: number }) => data)
  .handler(async ({ data }) => {
    ...
    const existing = await db
      .select()
      .from(months)
      .where(and(eq(months.year, data.year), eq(months.monthNo, data.monthNo), isNull(months.deletedAt)))
      .limit(1)
    if (existing.length > 0) return existing[0]

    const [created] = await db.insert(months).values({ year: data.year, monthNo: data.monthNo, createdBy: data.userId ?? null }).returning()
    return created
  })

export const updateMonth = createServerFn({ method: 'POST' as const })
  .validator((data: { id: number; year?: number; monthNo?: number; userId?: number }) => data)
  .handler(async ({ data }) => {
    ...
    const [updated] = await db.update(months).set({ ...fields, updatedBy: userId ?? null }).where(eq(months.id, id)).returning()
    return updated
  })
...
export const closeMonth = createServerFn({ method: 'POST' as const })
  ...
  .handler(async ({ data }) => {
    const [updated] = await db.update(months).set({ closed: true, updatedBy: data.userId ?? null }).where(eq(months.id, data.id)).returning()
    return updated
  })
...
export const reopenMonth = createServerFn({ method: 'POST' as const })
  .validator((data: { id: number; userId?: number }) => data)
  .handler(async ({ data }) => {
    const [updated] = await db.update(months).set({ closed: false, updatedBy: data.userId ?? null }).where(eq(months.id, data.id)).returning()
    return updated
  })

export const setManager = createServerFn({ method: 'POST' as const })
  .validator((data: { monthId: number; memberId: number; userId?: number }) => data)
  .handler(async ({ data }) => {
    ...
    const [updated] = await db.update(months).set({ managerId: data.memberId, updatedBy: data.userId ?? null }).where(eq(months.id, data.monthId)).returning()
    return updated
  })
...
export const deleteMonth = createServerFn({ method: 'POST' as const })
  .validator((data: { id: number; userId?: number }) => data)
  .handler(async ({ data }) => {
    await db.update(months).set({ deletedAt: new Date(), deletedBy: data.userId ?? null }).where(eq(months.id, data.id))
    return { success: true }
  })
